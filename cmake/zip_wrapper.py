...
"""
    ...
    def trimPath(path):
        ...
        # always treat slashes as separators
        archivePath = archivePath.replace("/", os.path.sep)
        if parentDir:
            archivePath = archivePath.replace(os.path.sep, "", 1)
        if not includeDirInZip:
            archivePath = archivePath.replace(dirToZip + os.path.sep, "", 1)
        return os.path.normpath(archivePath)
    ...
    for (archiveDirPath, dirNames, fileNames) in os.walk(dirPath):
        for fileName in fileNames:
            ...
            if not os.name == 'nt' and os.path.islink(filePath):
                dest = os.readlink(filePath);
                zipInfo = zipfile.ZipInfo()
                zipInfo.filename = trimPath(filePath)
                zipInfo.create_system = 3
                zipInfo.external_attr = 0120755 << 16L # symlink, permissions lrwxr-xr-x
                outFile.writestr(zipInfo, trimPath(dest))
            else:
        ...
        if not fileNames and not dirNames:
            ...
            zipInfo.external_attr = 040755 << 16L # folder, permissions drwxr-xr-x
            outFile.writestr(zipInfo, "")
    outFile.close()

def any_file_newer_than(dirPath, ref_date):
...
# the zip file is only generated if at least one file in the source directory
# has a newer modification date than the target zip file
needs_update = not os.path.isfile(zip)
for dir in dirs:
    if needs_update:
        break
    needs_update = any_file_newer_than(dir, os.stat(zip).st_mtime)

if needs_update:
    for dir in dirs:
        zipdir(dir, zip, False)
else:
    print "Archive %s is up-to-date." % zip
